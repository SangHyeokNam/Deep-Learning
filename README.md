...
## 5. Loss functions
* 손실 함수는 내 신경망이 예측값과 실제 값의 손실(오차)이 있는지, 손실이 있다면 얼마나 있는 지 확인하기 위해 사용합니다.
> **손실 함수 종류**
> * 손실 함수에는 여러 종류가 있지만, 대표적으로 시그모이드 함수와 함께 이진 분류에 사용되는 Binary Cross-entropy, 보통 Softmax 함수와 함께 다중 분류에 사용되는 Categorical Cross-entropy, 예측한 값과 실제 값 사이의 평균 제곱 오차를 정의하는 평균 제곱 오차 Mean Squared Error(MSE)가 있습니다.
